...
typedef struct MC_BR_GroupMoveCyclicPosition
{
	/* VAR_INPUT (analog) */
	MCAxesGroupType* AxesGroup;	/**< The axis group reference establishes the connection between the function block and the axis group. */
	MCCoordinateSystemEnum CoordSystem;	/**< Selected coordinate system (see Coordinate systems) */
	double CyclicPosition[15];	/* Array of positions in ACS order */
	struct MCAdvancedMoveCycParType AdvancedParameters[15];
	MCInterpolationModeEnum InterpolationMode;	/* Array of moving modes */
	/* VAR_OUTPUT (analog) */
	signed long ErrorID;	/* Error number */
	/* VAR (analog) */
	struct MCInternalType Internal;
	/* VAR_INPUT (digital) */
	plcbit Enable;	/**< Execution of the function block begins on a rising edge of this input. */
	plcbit DisableAxis[15];
	/* VAR_OUTPUT (digital) */
	plcbit Valid;	/* Function block is ready for movement */
	plcbit Busy;	/**< Function block is active and must continue to be called. */
	plcbit CommandAborted;	/**< Command aborted by another command. */
	plcbit Error;	/**< Execution error */
} MC_BR_GroupMoveCyclicPosition_typ;
...
_BUR_PUBLIC void MC_BR_GroupMoveCyclicPosition(struct MC_BR_GroupMoveCyclicPosition* inst);
