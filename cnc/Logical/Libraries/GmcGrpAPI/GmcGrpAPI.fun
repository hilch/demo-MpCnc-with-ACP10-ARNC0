...
FUNCTION_BLOCK MC_BR_GroupMoveCyclicPosition (* This function block commands a cyclic group movement .*)
	VAR_INPUT
		AxesGroup : REFERENCE TO MCAxesGroupType; (*The axis group reference establishes the connection between the function block and the axis group.*)
		Enable : BOOL; (*The function block is active as long as this input is set.*)
		CoordSystem : MCCoordinateSystemEnum; (*Selected coordinate system (see Coordinate systems)*)
		CyclicPosition : ARRAY[0..14] OF LREAL; (*Array of positions in ACS order*)
		AdvancedParameters : ARRAY[0..14] OF MCAdvancedMoveCycParType;
		DisableAxis : ARRAY[0..14] OF BOOL;
		InterpolationMode :  MCInterpolationModeEnum; (*Array of moving modes*)
	END_VAR
	VAR_OUTPUT
		Valid : BOOL; (*Function block is ready for movement *)
		Busy : BOOL; (*Function block is active and must continue to be called.*)
		CommandAborted : BOOL; (*Command aborted by another command.*)
		Error : BOOL; (*Execution error*)
		ErrorID : DINT; (*Error number*)
...
FUNCTION_BLOCK MC_BR_OnlinePathInfluence_15 (*Executes online path influence.*)
	VAR_INPUT
		AxesGroup : REFERENCE TO MCAxesGroupType; (*The axis group reference establishes the connection between the function block and the axis group.*)
		Enable : BOOL; (*The function block is active as long as this input is set.*)
		CoordSystem : MCCoordinateSystemEnum ; (*Reference coordinate system for the shift.*)
		Position : ARRAY[0..14] OF LREAL; (*Array[0..N-1] of relative distances in every dimension of the selected coordinate system.*)
	END_VAR
	VAR_OUTPUT
		Enabled : BOOL; (*Function block is active and must continue to be called.*)
		Busy : BOOL; (*Function block is active and must continue to be called.*)
		CommandAborted : BOOL; (*Command aborted by another command.*)
		Error : BOOL; (*Execution error*)
		ErrorID : DINT; (*Error number*)
