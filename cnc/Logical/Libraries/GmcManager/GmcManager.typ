TYPE
	...
	(
		...
		mcERROR_COAST_TO_STANDSTILL_CMD, (*an axis error is generated on the drive, any active movement is coasted to standstill and the controller is switched off*)
		mcERROR_INDUCTION_HALT_CMD (*an axis error is generatedon the drive, any active movement is stopped with an induction halt and the controller is switched off*)
	);
	...
	(
		mcOM_M1_STOP :=0, (*Optional stop*)
		mcOM_FEED_FORWARD :=1, (*Feed forward*)
		mcOM_OPTMOT :=2, (*Jerk-limiting trajectory planner*)
		mcOM_SAVE_RESTARTDATA :=4, (*Save data to restart a program*)
		mcOM_MP_LOG :=5 (* Motion packet log *)
	);

	MCUnloadProgramModeEnum :
	(
		mcUPRG_NAME, (*Unload program by name*)
		mcUPRG_ALL (*Unload all programs*)
	);
	...
	(
		mcIPLM_DEFAULT,					(*default interpolation mode*)
	...
	(
		...
		mcPRGSP_PATH_DISTANCE_LINE	(*Path Distance Line*)
	);
	...
	McStopModeEnum :
	(
		mcNORMAL_STOP,		(*AxisGroup considers jerk limits during stop*)
		mcQUICK_STOP		(*AxisGroup ignores jerk limits during stop*)
	);
	...
	IGmcControlIf_csub : STRUCT (*Partial interface type (C only)*)
		vtable : DWORD;
	END_STRUCT;

	IGmcControlIf : IGmcControlIf_csub;
	...
	MCInternalType : STRUCT (*internal variable*)
		...
		Flags : USINT;
		ControlIf : REFERENCE TO IGmcControlIf;
		SeqNo : DINT;
	END_STRUCT;
	...
	MCInternalTwoRefType : STRUCT (*internal variable*)
		...
		ControlIf : REFERENCE TO IGmcControlIf;
		SeqNo : DINT;
		MaControlIf : REFERENCE TO IGmcControlIf;
		MaSeqNo : DINT;
	END_STRUCT;
